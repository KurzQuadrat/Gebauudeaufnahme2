...
Hinweis Baualtersklassen-Vorschlagswerte (Bundesanzeiger): Für die Heizlast-Standardwerte stehen optionale U-Wert-Vorschläge aus der „Bekanntmachung der Regeln zur Datenaufnahme und Datenverwendung im Wohngebäudebestand" vom 8. Oktober 2020 (BAnz AT 04.12.2020 B1, Tabelle 2 „Opake Bauteile" / Tabelle 3 „Transparente Bauteile") zur Verfügung. Quelle, übernommene Kategorien und der ausdrückliche Hinweis, dass es sich um Pauschal-/Erfahrungswerte und keinen normativen Heizlastnachweis handelt, sind in `docs/quellen/README.md` dokumentiert; die kontrolliert erzeugte Datenstruktur liegt in `js/uwerte-bundesanzeiger.js` (ausschließlich Werte, die eindeutig aus der Quelle entnommen werden konnten – nichts geschätzt oder ergänzt). In der Projektansicht kann eine „Baualtersklasse Gebäude" gewählt (oder per Button aus dem Baujahr abgeleitet) und je U-Wert-Bauteilgruppe (Außenwand, Dach, Bodenplatte, Boden/Decke, Fenster, Türen) die passende Quellen-Bauteilkategorie sowie optional ein Sanierungsjahr hinterlegt werden. Logik je Bauteil: Ist ein Sanierungsjahr gesetzt, wird die Baualtersklasse für DIESES Bauteil aus dem Sanierungsjahr abgeleitet (überschreibt für dieses Bauteil das Baujahr des Gebäudes); ohne Sanierungsjahr gilt die Baualtersklasse des Gebäudes. Für Fenster wird wegen der abweichenden Klassengrenzen aus Tabelle 3 unabhängig von der Gebäude-Baualtersklasse direkt aus Sanierungsjahr/Baujahr abgeleitet. Der Button „U-Werte aus Baualtersklasse übernehmen" (`uebernehmeUWerteAusBaualtersklasse()`) trägt Vorschlagswerte ausschließlich in aktuell LEERE U-Wert-Felder ein – vorhandene manuelle Werte werden nie automatisch überschrieben; bereits gefüllte Felder werden in der Rückmeldung als „nicht verändert" ausgewiesen. Manuelle Projektwerte und Raum-Overrides bleiben in jedem Fall überschreibbar und haben in der bestehenden Prioritätskette weiterhin Vorrang.
