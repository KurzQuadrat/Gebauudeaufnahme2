...
Hinweis getrennte U-Wert-Datenquellen EnEV/GEG vs. Bundesanzeiger (seit v0.2.43-dev): Es gibt zwei fachlich getrennte, NICHT zu vermischende U-Wert-Datenquellen. `js/uwerte-bundesanzeiger.js` enthält Bestands-/Baualterswerte (Erfahrungswerte für typische Bauteile im Gebäudebestand nach BAnz AT 04.12.2020 B1) und wird aktuell für die Heizlast-Bestandsabschätzung genutzt. `js/uwerte-enev-geg.js` enthält dagegen gesetzliche Mindestanforderungen (Umax) bei Änderung/Sanierung von Außenbauteilen nach EnEV Anlage 3 (2002–2020) bzw. GEG Anlage 7 (ab 01.11.2020), strukturiert in drei Perioden (A: 2002–2009, B: 2009–2014, C: ab 2014). Diese EnEV/GEG-Werte sind KEIN automatisch vorhandener Bauteilwert, KEINE BEG-/Förderanforderung und KEINE Neubauanforderung — sie beschreiben nur den gesetzlichen Höchstwert, der bei einer Sanierungsmaßnahme einzuhalten ist (10-Prozent-Regel beachten). Nach Quellenstand (`docs/quellen/enev-geg/auswertung/enev-geg-uwerte-vorarbeit.md`) ist GEG Anlage 7 seit Inkrafttreten am 01.11.2020 in keiner Novelle (2022/2023/2026) geändert worden. Die Datei ist in `index.html` eingebunden, wird aber in diesem Schritt NICHT in die Heizlast-U-Wert-Ableitung (`leiteUWerteAusBaujahrUndSanierungAb()`) integriert — keine automatische Übernahme, kein Vergleich, keine Warnlogik. Eine spätere Nutzung in der UI muss klar als gesetzlicher Mindestanforderungs-/Vergleichswert (nicht als vorhandener Ist-Wert) gekennzeichnet werden.
